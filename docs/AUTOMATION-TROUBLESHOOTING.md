...
### Canonical pem↔person mapping (for #server-channel)

Use this mapping (and any learned additions in memory) to resolve pems and server-name hints to Slack @mentions:

| Pem or server-name hint | Person (Slack @mention) |
|-------------------------|------------------------|
| pem: `/Users/lucasgautheron/cap` | Lucas Gautheron @Lucas Gautheron |
| pem: `/Users/elif.celen/cap` or server name contains `elf` | Elif Celen @Elif Celen |
| pem: `/Users/orangekiller/cap` or server name contains `chenyi` | Chenyi Li @Chenyi Li |
| pem: `cap` and server name contains `haoyu` | Haoyu Hu @Haoyu Hu |
| pem: `cap` and server name contains `kj338` or `zohar` | Naomi Havron @Naomi Havron |
| pem: `/Users/qkzhong/.ssh/psynet` or server name contains `qkzhong` | Qiankun Zhong @Qiankun Zhong |
| server name contains `yoko` | Yoko Urano @Yoko Urano |
| pem: `/Users/edgarjoseandradelotero/cap` or similar path under that username, or server name contains `edgar` | Edgar Andrade @Edgar Andrade |

For generic `pem: cap` with no name in the server, use server-name hints (e.g. haoyu, chenyi, elf, kj338, zohar, yoko, edgar) to resolve. `world-listen` and `ec2-globalpop` have no person mapping unless learned.
...
```
You manage EC2 instance tracking and notifications. Use the canonical pem↔person mapping in this repo's docs (and any mappings stored in memory) to resolve who owns which servers.
...
3. Send to Slack the main report (Running / Stopped / Long-running lists with names and pem).
4. Per-person follow-up: Using the canonical pem↔person mapping (see docs) and server-name hints for generic `pem: cap`, build for each person the list of their running and stopped server names. Then either:
   - Post a single follow-up message in the same channel that tags each person with their summary, e.g. "@Lucas Gautheron you have 2 server(s) running and 9 server(s) stopped. Running: lucas-n, lucas-control. Stopped: lucas-patterns1, lucas-niche, …" (one line or block per person), or
   - DM each person with that same summary (running count, stopped count, and server names). Prefer tagging in the channel unless DMs are required.
   If a pem or server has no mapping, skip that server for per-person messaging (or ask in the channel who it belongs to and remember).
5. Store this run's output in memory for next week's long-running comparison. Optionally ask in the channel for any pems that have no person mapping and remember replies.

**On Slack message trigger (when user replies with a person's name):**
1. Extract the person's name from the message
2. Look up which servers (running/stopped) correspond to that person's pem in the canonical mapping and your learned knowledge base
3. If you know the mapping, send them a direct Slack message with the reminder (running/stopped server names or "None"), or tag them in the channel with the summary
